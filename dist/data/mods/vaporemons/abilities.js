...
const Abilities = {
  ...
  deathaura: {
    ...
    onStart(pokemon) {
      let activated = false;
      for (const target of [.../* @__PURE__ */ new Set([...pokemon.alliesAndSelf(), ...pokemon.adjacentFoes()])]) {
        if (!activated) {
  ...
  adaptability: {
    onModifySTAB(stab, source, target, move) {
      if (move.forceSTAB || source.hasType(move.type)) {
        if (move.type === source.teraType && source.baseSpecies.types.includes(source.teraType) && source.hasItem("terashard")) {
          return 2.25;
        }
        return 2;
      }
