...
const Moves = {
  ...
  healingstones: {
    ...
    condition: {
      ...
      onSwitchIn(pokemon) {
        if (pokemon.hasItem("heavydutyboots") || pokemon.hasAbility("overcoat") || pokemon.hasItem("dancingshoes") || pokemon.hasItem("mantisclaw")) return;
  ...
  parry: {
    ...
    condition: {
      ...
      onFoeTryMove(target, source, move) {
        ...
        if ((source.isAlly(parryHolder) || move.target === "all") && (!source.hasAbility("innerfocus") || !source.hasAbility("shielddust") || !source.hasAbility("steadfast") || !source.hasItem("covertcloak") || !source.hasAbility("sandveil") && !this.field.isWeather("sandstorm") || !source.hasAbility("sunblock") && !this.field.isWeather("sunnyday") || !source.hasAbility("snowcloak") && !this.field.isWeather("snowscape")) && move.priority > 0.1) {
          this.attrLastMove("[still]");
  ...
  stickyweb: {
    ...
    condition: {
      ...
      onSwitchIn(pokemon) {
        if (!pokemon.isGrounded() || pokemon.hasItem("heavydutyboots") || pokemon.hasItem("dancingshoes") || pokemon.hasAbility("overcoat") || pokemon.hasItem("mantisclaw")) return;
